...
function getPropsRecursive(req, match, ignore) {
  ...
  serialized = _qs2.default.stringify(pobj);
...
function resolveMockPath(req, dataRoot) {
  ...
  if (parts) {
    ...
    // Query string
    const props = !req.conf.matchPropsRecursive ? getProps(req, req.conf.matchProps, req.conf.ignoreProps) : getPropsRecursive(req, req.conf.matchProps, req.conf.ignoreProps);
  ...
  return (0, _path.join)(dataRoot, path + '.mock');
}
